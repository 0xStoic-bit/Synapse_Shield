...
def poisson_anomaly_score(k: int, lambda_val: float = 2.0) -> float:
    """
    Computes Poisson cumulative anomaly probability P(X < k).
    """
    if k <= 1:
        return 0.0
    cumulative_prob = 0.0
...
def analyze_behavior(telemetry: Dict[str, Any], recent_request_count: int = 1) -> Tuple[float, str, List[str], Dict[str, Any]]:
    ...
    # 3. Fare Hareketsiz Etkileşimler
    ...
    # 4. Kinematik Fare Hareket Analizi
    ...
    if freq_anomaly >= 0.95:
        ...
        if is_human_telemetry:
            # İnsan biyometrisi varsa frekans riski 25% ile sınırlanır (tek başına engellemez)
            ...
            # Telemetrisi olmayan bot / headless flooder ise engellenir (+60 risk)
