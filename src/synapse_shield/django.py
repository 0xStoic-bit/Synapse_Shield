...
class SynapseShieldMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        try:
            ...
            return JsonResponse(
                {"error": "[Synapse Shield] Missing or invalid behavioral telemetry payload."}, status=403
            )

        # Execute synchronously (Django's default blocking model)
        bot_score, classification, reasons, _ = analyze_behavior(telemetry, 1, False, self.accessibility_mode)
