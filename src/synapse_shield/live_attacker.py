...
def print_result(attack_num: int, title: str, res: dict, expected_blocked: bool = True) -> bool:
    ...
    if success:
        status_text = f"{C.GREEN}✅ KALKAN BAŞARILI (Engellendi){C.END}" if expected_blocked else f"{C.GREEN}✅ KALKAN BAŞARILI (İzin verildi){C.END}"
    else:
        status_text = f"{C.RED}❌ BAŞARISIZ{C.END}"
...
def main():
    ...
    print(f"{C.BOLD}{C.YELLOW}║   🔴 SYNAPSE SHIELD — RED TEAM BOT SALDIRI SÜİTİ            ║{C.END}")
    ...
    results.append(print_result(1, "Selenium Headless Crawler", res1, expected_blocked=True))

    # 2. Linear Mouse
    t = int(time.time()*1000)
    res2 = send_attack("Linear", {"mouse_movements": [{"x": 50 + i*30, "y": 50 + i*20, "t": t + i*20} for i in range(25)]}, ip_suffix=20)
    results.append(print_result(2, "Doğrusal Fare Botu (Straight-Line)", res2, expected_blocked=True))
    ...
    results.append(print_result(3, "Bézier Eğrisi Botu (No-Jerk Curve)", res3, expected_blocked=True))

    # 4. Auto-Typer
    keys = [{"type": "down", "t": t + i*100} for i in range(10)]
    res4 = send_attack("AutoTyper", {"keystrokes": keys, "clicks": [{"x": 100, "y": 100, "t": t}]}, ip_suffix=40)
    results.append(print_result(4, "Robotik Klavye Otomatı", res4, expected_blocked=True))
    ...
    results.append(print_result(5, "Poisson İstek Bombardımanı (DDoS)", res5, expected_blocked=True))
    ...
    results.append(print_result(6, "Doğal İnsan Ziyaretçisi (Control)", res6, expected_blocked=False))

    # 7. Replay Attack Simülasyonu
    print(f"\n{C.YELLOW}[*] Replay Attack Testi: Gerçek bir token çalınıp 2. kez gönderiliyor...{C.END}")
    ch = get_challenge()
    if ch:
        envelope = {"challenge": ch, "telemetry": {"mouse_movements": human_pts}, "created_at": int(time.time())}
        ...
        results.append(print_result(7, "Replay Attack (Aynı Token'ı Tekrar Kullanma)", res7, expected_blocked=True))
    else:
        print(f"{C.RED}[!] Challenge alınamadı; Replay testi atlandı.{C.END}")
    ...
    print(f"\n{C.BOLD}{C.CYAN}══════════════════════════════════════════════════════════════{C.END}")
    if passed == total:
        print(f"  {C.BOLD}{C.GREEN}🎯 TÜM {total} SALDIRI VEKTÖRÜ ENGELLENDİ: {passed}/{total}{C.END}")
    else:
        print(f"  {C.BOLD}{C.RED}⚠️  {total - passed} SALDIRI KALKANI GEÇTİ: {passed}/{total} başarılı{C.END}")
    print(f"{C.BOLD}{C.CYAN}══════════════════════════════════════════════════════════════{C.END}\n")
