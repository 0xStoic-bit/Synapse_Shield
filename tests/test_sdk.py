...
def test_sdk_static_serving():
    """Verify that the SDK static endpoint serves the updated v0.7.2 JavaScript SDK."""
    response = client.get("/static/synapse-sdk.js")
    assert response.status_code == 200
    assert "application/javascript" in response.headers.get("content-type", "")
    content = response.text
    assert "Synapse Shield SDK v0.7.2" in content
...
def test_vue_sdk_payload_compatibility():
    """Verify that payloads generated by the Vue 3 SDK (synapse-shield-vue) format are verified seamlessly."""
    chal_res = client.get("/api/challenge")
    assert chal_res.status_code == 200
    challenge = chal_res.json()["challenge"]

    # Typical structure emitted by synapse-shield-vue
    telemetry = {
        "mouse_movements": [
            {"x": 120, "y": 250, "t": int(time.time() * 1000) - 200},
            {"x": 125, "y": 255, "t": int(time.time() * 1000) - 150},
        ],
        "clicks": [{"x": 125, "y": 255, "t": int(time.time() * 1000) - 100}],
        "keystrokes": [{"type": "down", "t": int(time.time() * 1000) - 50}],
        "scrolls": [],
        "browser": {
            "webdriver": False,
            "screen_width": 1440,
